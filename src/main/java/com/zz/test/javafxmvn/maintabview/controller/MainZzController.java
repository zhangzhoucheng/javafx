...
public class MainZzController extends BaseObjectViewOth{
	...
	private MainZzService mainZzService = (MainZzService) SpringUtils.getBean("mainZzService");
	...
	public void loginSerch(ActionEvent event) throws InstantiationException, IllegalAccessException {
		...
		thePagination = TagTool.TableTool.tablePagePagination(10,"mainZzService.loginSerchnew", request, fieldsHead.getTableHeadFields(), width, false);
	...
	public EventHandler<ActionEvent>  addButtonClick() {
		...
		return  (ActionEvent e) -> {
			...
			int insertRet = -1;
			try {
				insertRet = mainZzService.insertPyProcess(addP);
			} catch(Exception e1){
		...
		return (ActionEvent e) -> {
			...
			while(itpy.hasNext()){
				...
				if(p.getChoice().getValue() == true && "取消编辑".equals(editButton.getText())) {
					int ret = mainZzService.updatePyProcessList(p);
	...
	public EventHandler<ActionEvent> deleteButton() {
		...
		return (ActionEvent e) -> {
			...
			while(itpy.hasNext()){
				PyProcess p = itpy.next();
				if(p.getChoice().getValue() == true && "取消编辑".equals(editButton.getText())) {
					int ret = mainZzService.deleteByPrimaryKeySet2(p);
					p.getChoice().set(false);
					itpy.remove();
				}
				i ++ ;
