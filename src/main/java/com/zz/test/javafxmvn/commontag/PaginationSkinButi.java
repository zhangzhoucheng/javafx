 ...
 * 
 * <note>
 * Desc： 对PaginationSkin 的initializePageIndicators 方法进行了重构，因为我想为分页插件加上 选择页数的内容。
 * @author jld.zhangzhou
 * @email idiot_jillidan@163.com;
 * @re be willing to communicate
 * @refactor for jld
 * @datetime 2020-05-09 15:09:22
 ...
 * ------------------------------------------------------------------------------------------------------  
 * 1.0 		  2020-05-09 15:09:22    jld.zhangzhou     mobile base 3th,BeiJing      1.create the class            
 * </note>
 */
public class PaginationSkinButi extends BehaviorSkinBase<Pagination, PaginationBehavior>  {
    ...
    public PaginationSkinButi(final Pagination pagination) {
...
//        setManaged(false);
    ...
    private final DoubleProperty arrowButtonGap = new StyleableDoubleProperty(60.0) {
        @Override public Object getBean() {
            return PaginationSkinButi.this;
        }
    ...
    class NavigationControl extends StackPane {
        ...
        private void initializePageIndicators() {
            ...
            /**
             * 重构新增部分。jldzz-20200509
             */
            List<Node> bakNode = new ArrayList<>();
            int boxSize = controlBox.getChildren().size();
            if(boxSize > 2) {
            	int i = 0;
            	int btnFlag = 0;
            	int ofBtn = 0;
            	for(Object o : controlBox.getChildren()) {
            		if(o instanceof Button && ! (o instanceof IndicatorButton)) {
            			btnFlag ++ ;
            		}
            		if(btnFlag == 2) {
            			ofBtn = i;
            	...
            	while(controlBox.getChildren().size() > (ofBtn + 1)) {
            		bakNode.add(controlBox.getChildren().get(ofBtn + 1));
            		ofBtn ++;
            	}
            ...
            if(bakNode.size() > 0) {
            	controlBox.getChildren().addAll(bakNode);
            }
        }
...
//            }
            ...
            double controlBoxX = left + UtilsMe.computeXOffset(width, controlBoxWidth, controlBoxHPos);
            double controlBoxY = top + UtilsMe.computeYOffset(height, controlBoxHeight, controlBoxVPos);

            if (isPageInformationVisible()) {
                Pos p = sideToPos(getPageInformationAlignment());
                HPos pageInformationHPos = p.getHpos();
                VPos pageInformationVPos = p.getVpos();
                double pageInformationX = left + UtilsMe.computeXOffset(width, pageInformationWidth, pageInformationHPos);
                double pageInformationY = top + UtilsMe.computeYOffset(height, pageInformationHeight, pageInformationVPos);
    ...
    private static class StyleableProperties {
        private static final CssMetaData<Pagination,Boolean> ARROWS_VISIBLE =
            ...
            public boolean isSettable(Pagination n) {
                final PaginationSkinButi skin = (PaginationSkinButi) n.getSkin();
    ...
    //由于Utils有包访问权限，所以在此继承，隐藏方法。
    public static class UtilsMe extends Utils{
